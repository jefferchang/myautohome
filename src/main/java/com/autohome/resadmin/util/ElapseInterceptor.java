...
public class ElapseInterceptor {
    ...
    private static Logger ELAPSE_LOG = Logger.getLogger("elapseLogger");

    private static Logger ELAPSE_TIMEOUT_LOG = Logger.getLogger("elapseTimeOutLogger");
    ...
    private void writeToLog(String topic, long elapsedMilliseconds) {
        ...
        ELAPSE_LOG.warn(topic + "`" + elapsedMillisecondsStringFormat);

        if (elapsedMilliseconds > 2000) {
            ELAPSE_TIMEOUT_LOG.fatal(getErrorLogMessage(elapsedMillisecondsStringFormat, topic));
        } else if (elapsedMilliseconds > 1000) {
            ELAPSE_TIMEOUT_LOG.error(getErrorLogMessage(elapsedMillisecondsStringFormat, topic));
        } else if (elapsedMilliseconds > 500) {
            ELAPSE_TIMEOUT_LOG.warn(getErrorLogMessage(elapsedMillisecondsStringFormat, topic));
        }
    }
